...
class UsersTableSeeder extends Seeder
{
    public function run(): void
    {
        ...
        (new WalletService)->transfer($senior, $owner, 5 * 100_0000, TransactionName::CreditTransfer);

        $agent = $this->createUser(UserType::Agent, 'Agent 1', 'A898737', '09112345674', $owner->id, 'vH6u5E9');
        (new WalletService)->transfer($owner, $agent, 5 * 100_0000, TransactionName::CreditTransfer);

        $player = $this->createUser(UserType::Player, 'Player 1', 'Player001', '09111111111', $agent->id);
        (new WalletService)->transfer($agent, $player, 5 * 100_0000, TransactionName::CreditTransfer);
        ...
        (new WalletService)->transfer($agent, $player1, 5 * 100_0000, TransactionName::CreditTransfer);
        $player2 = $this->createUser(UserType::Player, 'Player3', 'Player003', '09111111113', $agent->id);
        (new WalletService)->transfer($agent, $player2, 5 * 100_0000, TransactionName::CreditTransfer);
        $player3 = $this->createUser(UserType::Player, 'Player4', 'Player004', '09111111114', $agent->id);
        (new WalletService)->transfer($agent, $player3, 5 * 100_0000, TransactionName::CreditTransfer);
        $player4 = $this->createUser(UserType::Player, 'Player5', 'Player005', '09111111115', $agent->id);
        (new WalletService)->transfer($agent, $player4, 5 * 100_0000, TransactionName::CreditTransfer);
    }
